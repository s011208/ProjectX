...
public class OrderedAdapter extends BaseAdapter implements DataLoadTask.Callback {
    ...
    public OrderedAdapter(Context context, final int gameType) {
        GAME_TYPE = gameType;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = mInflater.inflate(R.layout.ordered_table_rows, null);
        ...
        if (position == 0) {
            ...
            for (int i = 1; i < container.getChildCount(); i++) {
                holder.mText.get(i - 1).setText(String.valueOf(Utils.NUMBER_FORMATTER.format(i)));
            }
        } else {
            LotteryData data = getItem(position);
            holder.mDate.setText(data.mDate);
            for (int i = 1; i < container.getChildCount(); i++) {
                if (AWARDS_NUMBER_COUNT >= 7) {
                    if (data.m1 == i || data.m2 == i || data.m3 == i || data.m4 == i
                            || data.m5 == i || data.m6 == i || data.m7 == i) {
                        holder.mText.get(i - 1).setText(
                                String.valueOf(Utils.NUMBER_FORMATTER.format(i)));
                    } else {
                    ...
                    if (data.m1 == i || data.m2 == i || data.m3 == i) {
                        holder.mText.get(i - 1).setText(
                                String.valueOf(Utils.NUMBER_FORMATTER.format(i)));
                    } else {
                        holder.mText.get(i - 1).setText(null);
